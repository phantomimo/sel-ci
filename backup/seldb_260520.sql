...
-- Tiempo de generación: 26-05-2020 a las 16:31:49
...
CREATE DEFINER=`u899190196_instituto`@`127.0.0.1` PROCEDURE `OBTENER_PARAMETRO` (IN `P_NOMBRE` VARCHAR(35), IN `P_VALOR_DEFECTO` VARCHAR(100), OUT `O_PARAMETRO` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`u899190196_instituto`@`127.0.0.1` PROCEDURE `OBTENER_PREFERENCIA_USUARIO` (IN `P_CLAVE_USUARIO` INT, IN `P_CLAVE_SUCURSAL` INT, IN `P_NOMBRE` VARCHAR(45), IN `P_VALOR_DEFECTO` VARCHAR(45), OUT `O_VALOR` VARCHAR(45))  BEGIN
...
(1, '', 'Pensamiento matemático', '', NULL, 2, '2020-05-19 00:00:00', '2020-05-19 17:03:07'),
(2, 'CL001', 'Comprensión lectora', '', NULL, 5, '2020-05-19 09:33:18', '2020-05-22 03:36:08'),
(3, '', 'Pensamiento analítico', '', NULL, 2, '2020-05-19 09:33:14', '2020-05-19 17:03:04'),
(4, '', 'Estructura de la lengua', '', NULL, 5, '2020-05-19 09:33:02', '2020-05-19 17:03:02'),
...
(42, 7, 1, 1, '¿Qué rama de la biología estudia a los peces?', 'Herpetología          ', 'Entomología                    ', 'Ictiología                   ', 'Ornitología', 3, 0, '2020-05-21 11:26:28', '2020-05-26 15:12:56'),
(43, 7, 1, 2, '¿Qué rama de la biología estudia las funciones de los seres vivos?', 'Ecología                          ', 'Taxonomía                        ', 'Genética                    ', 'Fisiología ', 4, 0, '2020-05-21 11:27:40', '2020-05-26 15:14:56'),
...
(52, 8, 1, 1, 'Es la ciencia que estudia a la materia, sus propiedades, composición, reactividad y las transformaciones que experimentan, es:', 'CIENCIAS NATURALES         ', 'FISICA            ', 'QUIMICA    ', 'BIOQUIMICA ', 3, 0, '2020-05-24 20:55:48', '2020-05-26 15:16:07'),
...
ALTER TABLE `examenes_alumnos`
  ...
  ADD KEY `id_examen` (`id_examen`) USING BTREE;
...
--
ALTER TABLE `alumnos`
  ADD CONSTRAINT `alumnos_ibfk_1` FOREIGN KEY (`id_nivel`) REFERENCES `niveles` (`id_nivel`) ON UPDATE CASCADE,
  ADD CONSTRAINT `alumnos_ibfk_2` FOREIGN KEY (`id_grado`) REFERENCES `grados` (`id_grado`) ON UPDATE CASCADE,
  ADD CONSTRAINT `alumnos_ibfk_3` FOREIGN KEY (`id_grupo`) REFERENCES `grupos` (`id_grupo`) ON UPDATE CASCADE;

--
-- Filtros para la tabla `asignaturas_niveles`
--
ALTER TABLE `asignaturas_niveles`
  ADD CONSTRAINT `asignaturas_niveles_ibfk_1` FOREIGN KEY (`id_asignatura`) REFERENCES `asignaturas` (`id_asignatura`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `asignaturas_niveles_ibfk_2` FOREIGN KEY (`id_nivel`) REFERENCES `niveles` (`id_nivel`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `examenes_alumnos`
  ADD CONSTRAINT `examenes_alumnos_ibfk_1` FOREIGN KEY (`id_examen`) REFERENCES `examenes` (`id_examen`) ON UPDATE CASCADE,
  ADD CONSTRAINT `examenes_alumnos_ibfk_2` FOREIGN KEY (`id_alumno`) REFERENCES `alumnos` (`id_alumno`) ON UPDATE CASCADE;
